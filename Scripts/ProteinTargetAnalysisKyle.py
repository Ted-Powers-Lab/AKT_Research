...
def makeTempFasta(proteomeDir):
    targetName = None
    accession = None
    substring = "GC"

    for proteome in pathlib.Path(proteomeDir).rglob('*.faa'):

        nameParts = proteome.parts
        accession = list(s for s in nameParts if substring in s)
        accession = ", ".join(accession)
        with open(proteome, 'r') as prot:
            for line in prot:
                line = line.rstrip()
                if not line:
                    continue
...
def main():
    ...
    parser.add_argument('-prot', help='Proteome Directory')
    ...
    proteomeDir = pathlib.Path(arg.prot)

    print("About to run the function")
    makeTempFasta(proteomeDir)
